...
<div class="flex-row">
	...
	<c:url var="firstUrl" value="/search?page=0"/>${movies.totalPages} ${actorsList.totalPages}
	<c:choose>
		<c:when test="${ movies.totalPages > actorsList.totalPages}">
			<c:url var="lastUrl" value="/search?page=${ movies.totalPages - 1 }"/><p>${ movies.totalPages - 1 }"</p>
		</c:when>
		<c:otherwise>
			<c:url var="lastUrl" value="/search?page=${ actorsList.totalPages - 1 }"/><p>${ actorsList.totalPages - 1 }</p>
		</c:otherwise>
	</c:choose>
			...
			<div class="col-md-12">
				<table class="table table-hover table-striped">
					...
					<tbody>
						...
						<tr>
							<th colspan="5" style="text-align: center">List of movies</th>
						</tr>
						<tr>
							<th>Name</th>
							<th>Country</th>
					...
					<tbody>
						<c:forEach items="${ moviesByPageSize }" var="movie">
							<tr>
								<td style="vertical-align: middle;"><a
									href="${pageContext.request.contextPath}/movie/${movie.id}">${movie.name}</a>
								</td>
								<td style="vertical-align: middle;">
									<c:forEach items="${movie.country}" var="country">
										${country.name} &ensp;
									</c:forEach>
								</td>
								<td style="vertical-align: middle;">${ movie.year}</td>
								<sec:authorize access="hasAnyRole('ADMIN', 'MODERATOR')">
									<td style="vertical-align: middle; width: 20%">
										<div class="btn-group">
											<a type="button" class="btn btn-primary"
